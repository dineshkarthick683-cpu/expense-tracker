...
@Route("incomecharts")
public class IncomeChartView extends VerticalLayout {
    ...
    public IncomeChartView(IncomeService incomeService) {
...
//        emptyLayout1.setWidthFull();
    ...
    private void loadCharts() {
        ...
        List<AppIncomeMainViewModel> allExpenses =
                incomeService.getAllIncome();

        LocalDate start = startDate.getValue();
        LocalDate end = endDate.getValue();

        List<AppIncomeMainViewModel> filteredList =
                allExpenses.stream()
                        .filter(exp -> {

                            LocalDate expDate =
                                    exp.getIncomeDate().toLocalDate();
    ...
    private VerticalLayout createPieChart(
            List<AppIncomeMainViewModel> income) {

        Map<String, Double> categoryMap =
                income.stream()
                        .collect(Collectors.groupingBy(
                                AppIncomeMainViewModel::getCategory,
                                Collectors.summingDouble(
                                        AppIncomeMainViewModel::getAmount)));
    ...
    private VerticalLayout createLineChart(
        ...
        Map<LocalDate, Double> dailyMap =
                income.stream()
                        .collect(Collectors.groupingBy(
                                inc -> inc.getIncomeDate().toLocalDate(),
                                TreeMap::new,
                                Collectors.summingDouble(
                                        AppIncomeMainViewModel::getAmount
                                )
